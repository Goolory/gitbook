...
##### C++对象和实例的区别，以及用new和不用new创建类型对象区别？

<font color="red">c++ 对象实例化的一些概念：</font>

C++如果直接定义类，如classA a; a存在栈上（也意味着赋值了对象a在栈中）；
...
不能。如果序列号固定产生：假设ISN（序列号）固定为1，client和server建立一条TCP连接后，Client连续发生10个包给server，后这10个包被链路上的路由器缓存了，这时候碰巧Client挂了，重连，又发了5个包，与网络上缓存的10个包同时达到server此时就出现了问题。

RFC793中，建议ISN与一个假的时钟绑定在一起，这个时钟会每4微秒对ISN加一，到2^32次后回绕，--改进（取随机）
...
有可能，被动端接收到ACK后，如果没有数据要发送了，那他可以将自己的FIN和ACK绑定一起发送

##### 哪一段会进入TIME_WAIT？

主动方会进入

但是双方都是主动方的话他们都会进入TIME_WAIT

##### TIME_WAIT会带来什么问题？

1. 作为服务器，段时间内关闭了大量的client连接， 会造成服务器上出现大量的TIMET_WAIT连接，严重消耗着服务器的资源
2. 作为客户端，段时间内大量的短链接，会大量消耗client机器的端口，毕竟端口只有65535个，端口被消耗尽了，后续就无法在发起新的连接了。
...
延迟ACK，ACK在收到数据后并不马上回复，而是延迟一段可以接受的时间，看能不能和接受方要发送的数据一起发送回去，因此TCP协议头中总是包含了确认号。
...
是用户定义的一些列SQL语句的集合，涉及特定表或其他对象的任务。用户可以调用存储过程。而函数通常是数据库定义的方法（SUM 、COUNT等）
